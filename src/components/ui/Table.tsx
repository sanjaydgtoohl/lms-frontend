  ...
  return (
    <>
      ...
      <div className="hidden lg:block overflow-x-auto">
        <table className="w-full text-center">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {loading ? (
              <tr>
                <td colSpan={columns.length + (showActions ? 1 : 0)} className="px-6 py-16">
              ...
              data.map((item, index) => (
                <tr 
                  key={keyExtractor ? keyExtractor(item, index) : (String((item as any).id) || String(index))} 
      ...
      <div className="lg:hidden space-y-3">
        {loading ? (
          <div className="px-6 py-16 flex flex-col items-center justify-center space-y-3">
            <Loader2 className="w-10 h-10 animate-spin text-blue-500" />
            <p className="text-sm text-gray-500">Loading data...</p>
          </div>
        ) : !hasData ? (
          ...
          <div 
            key={keyExtractor ? keyExtractor(item, index) : (String((item as any).id) || String(index))} 
            className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md hover:border-blue-300 transition-all duration-200"
          >
            ...
            <div className="space-y-2.5">
              {columns.map(col => (
                <div 
                  key={col.key} 
                  className={`flex justify-between items-start gap-2 ${col.hideOnMobile ? 'hidden' : ''}`}
                >
                  <span className="text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap flex-shrink-0">
                    {col.header}:
                  </span>
